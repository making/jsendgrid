...
public final class JsonCodec {
	...
	private static final AsciiString APPLICATION_JSON = new AsciiString(
	...
	public static <T> Function<T, ByteBuf> encode(ObjectMapper objectMapper,
			HttpClientRequest request) {
		request.header(CONTENT_TYPE, APPLICATION_JSON);
		return source -> encode(request.delegate().alloc(), objectMapper, source);
	}

	static <T> ByteBuf encode(ByteBufAllocator allocator, ObjectMapper objectMapper,
			T source) {
		try {
